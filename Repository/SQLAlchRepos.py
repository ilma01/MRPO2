...
class ModelAdapter:
    @staticmethod
    def to_new_consumable(old_consumable: OldConsumable) -> Consumable:
        return Consumable(
    ...
    @staticmethod
    def to_new_service(old_service: OldService) -> Service:
        return Service(
            id=old_service.id,
            ...
            consumables=[ModelAdapter.to_new_consumable(consumable) for consumable in old_service.consumables]
        )
    @staticmethod
    def to_old_service(new_service: Service) -> OldService:
        if new_service:
            old_service = OldService(
                ...
                car=ModelAdapter.to_old_car(new_service.car),
                tasks=new_service.tasks,
                consumables=[ModelAdapter.to_old_consumable(consumable) for consumable in new_service.consumables]
            )
            old_service.mileage = new_service.mileage
            return old_service
        return None
    @staticmethod
    def to_old_consumable(new_consumable: Consumable) -> OldConsumable:
         if new_consumable:
...
class CarBrandRepos(FakeRepos):
    ...
    def get(self, name):
        return self.to_old_car_brand(self.session.query(CarBrand).filter_by(name=name).first())
...
class CarModelRepos(FakeRepos):
    ...
    def get(self, name):
        return self.to_old_car_model(self.session.query(CarModel).filter_by(name=name).first())

class CarRepos(FakeRepos):
    def to_new_car(old_car: OldCar) -> Car:
        return Car(
            model=ModelAdapter.to_new_car_model(old_car.model),
    ...
    def to_old_car(new_car: Car) -> OldCar:
        if new_car:
            return OldCar(
                model=ModelAdapter.to_old_car_model(new_car.model),
    ...
    def get(self, vin):
        if self.to_old_car(self.session.query(Car).filter_by(vin=vin).first()):
            return self.to_old_car(self.session.query(Car).filter_by(vin=vin).first())
        return None
...
class ConsumableRepos(FakeRepos):
    ...
    def update(self, consumable):
        self.session.merge(consumable)
        self.session.commit()

    def get_all(self):
        return self.session.query(Consumable).all()

    def get(self, name):
        return self.session.query(Consumable).filter_by(name=name).all()
